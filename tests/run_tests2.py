...
def run_tests(exec_name):
    for file in files:
        ...
        with open(name + '.myout', 'w') as outfile:
            ...
            result = subprocess.check_output(diff_command)
            print "{}Time passed: {:.2f} s".format(RESET, end - start)
            if result == b'':
...
if len(sys.argv) != 2:
    ...
    subprocess.call(serial_command)
    run_tests('./life3d')
    print "Done\n" 

elif sys.argv[1] == '-openmp':
    print "Running OpenMP version\n"
    subprocess.call(openmp_command)
